...
import { deleteAuthorPermanently } from "@/lib/admin/actions";
import { createClient } from "@/lib/supabase/server";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("admin.deletion");
  return { title: t("authorMetaTitle") };
}

/**
 * The permanent-deletion confirmation page for an author: dependency
 * summary from the database, typed confirmation of the current name,
 * and an explicit permanence acknowledgment. The layout above already
 * gates /admin to staff; the action and the RPC re-check.
 */
export default async function AdminAuthorDeletePage({
  params,
  searchParams,
}: {
  ...
  if (error) {
    console.error("[admin] author deletion preview failed", error);
    notFound();
  }
  if (!preview) notFound();
  ...
  return (
    <>
      ...
      <h1 className="mt-3 font-display text-4xl tracking-tight text-oxblood">
        {t("authorHeading", { name: preview.fullName })}
      </h1>
      <p className="mt-4 max-w-prose leading-relaxed text-ink-soft">
        {t("authorLede")}
      </p>
      {preview.hasLinkedUser ? (
        <p className="mt-3 max-w-prose font-sans text-sm text-oxblood">
          {t("linkedAccountWarning")}
        </p>
      ) : null}
      ...
      <section className="rule mt-10 max-w-xl pt-6">
        ...
        <form action={deleteAuthorPermanently} className="mt-4 space-y-6">
          <input type="hidden" name="author_id" value={preview.id} />
          ...
          <div>
            ...
            >
              {t("authorTypeToConfirm", { name: preview.fullName })}
            </label>
            <input
              id="confirmation"
              name="confirmation"
