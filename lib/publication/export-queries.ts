...
export const getExportHistory = cache(async function getExportHistory(
  ...
  const [artifactsResult, attemptsResult] = await Promise.all([
    supabase
      ...
      .select(
        "id, candidate_id, candidate_number, candidate_fingerprint, format, serializer, serializer_version, artifact_number, generated_at, checksum, byte_size, validator, validator_version, regenerates_artifact_id",
      )
  ...
  return {
    artifacts: (artifactsResult.data ?? []) as ArtifactRecord[],
